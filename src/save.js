...
window.MZ = window.MZ || {};
(() => {
  const KEY = 'mazmorra_tour_v1';
  const DB = 'mazmorra_tour';
  const STORE = 'kv';
  const defaults = { bestDepth: 0, totalKills: 0, totalGold: 0, deaths: 0, runs: 0, morena: 0, loreCap: 0, nona: 0, totalSteps: 0, run: null, codex: null, sellos: null, finalBueno: 0 };
  let data = { ...defaults };
  let db = null;
  ...
  MZ.save = {
    ...
    // borra todo el progreso (vuelve a cero)
    reset() {
      data = { ...defaults };
      this.store();
    },
